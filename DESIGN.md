...
- 当前一段状态先于整场总数，使用文字与状态色说明升温、平稳、回落、安静和缺口，不只显示百分比。
- 30分钟趋势按弹幕、活跃发言、热度、礼物、醒目留言分成五张图，固定使用红、黄、绿、蓝、紫，并保留标题、单位和当前值。
- 趋势中的数据缺口使用珊瑚红虚线格和说明文字，不把缺失区间绘制为正常零值。
- 高频词使用暖色，活跃用户使用冷色；栏目标题、左右位置与文字标签继续提供非颜色提示。
- 光谱颜色不能覆盖说明文字，正文和辅助文字继续使用暖灰分级。

### Post-live Review

- 历史详情先展示整场结论，再按整场统计、五类分时趋势、高峰时段、重复弹幕和下场验证排列。
- 复盘趋势沿用实时看板的红、黄、绿、蓝、紫指标对应，每张图同时保留指标名、峰值、图例和缺口虚线。
- 高峰时段排名使用共享分隔线的平面列表，不把每个时段包成独立圆角卡片。
- 下场验证清单中，「数据依据」和「下场验证」使用固定标签，不使用颜色替代结论层级。

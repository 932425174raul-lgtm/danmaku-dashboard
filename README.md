...
这个项目适合需要在直播过程中观察观众反应，或在直播结束后根据互动趋势做复盘的内容创作者、直播运营者和研究者。它不要求输入平台Cookie，不依赖主播账号，也不把历史上传到外部服务。
...
| 直播结束后 | 聚合整场互动并保留弹幕索引 | 整场结论、五类分时趋势、高峰时段排名、重复弹幕和下场验证清单 |
...
- 直播结束后自动生成「直播复盘」，首屏给出互动高峰时段、弹幕量、活跃发言人数和数据完整性结论。
- 分时展示弹幕量、活跃发言人数、平台热度峰值、礼物和醒目留言五类趋势；数据缺口不会被当作正常零值。
- 按弹幕量排出前5个高峰时段，同时显示活跃发言人数、全场占比和缺口状态。
- 精确统计完整文本相同的重复弹幕TOP 5，附带出现次数、发言人数和首末时间。
- 排除缺口时间格后比较前后半场的平均活跃发言人数，数据不足时不强行判断趋势。
- 下场验证清单会把「数据依据」和「下场验证」分开，引导用户回看当时的主题、话术或互动动作，不把数据关联冒充为因果。
...
![直播结束后的整场结论、高峰时段和下场验证清单](./docs/images/history-review-overview.png)

<sub>复盘截图使用合成数据，没有收录线上直播的真实用户或弹幕。</sub>
...
复盘中的“活跃发言人数”指一个时间格内发过弹幕的本机匿名去重人数，不是直播间在线观众人数；B站“直播热度”也不会被换算为人数。
...
6. 采集结束后，进入“历史”查看直播复盘。先定位互动高峰和重复弹幕，再按建议回看对应时段，也可以搜索原始弹幕或删除整场数据。
...
- 复盘页在1440px宽窗口和430px窄窗口中完整显示5张趋势图、5个高峰时段和4条验证动作，没有水平溢出或控制台错误。
